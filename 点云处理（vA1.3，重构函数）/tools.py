...
def pcd_to_point(pcd):
	point=cp.asarray(pcd.points).get()
	return point 

def Xyz_compute(result,point,ax):
	...
	if method_id==b'0150':
		...
		for num in range(0,NumPoint):                       
				...
				if HexInValidFlag=='00' and (HexAzAngle!='00000000' or HexELAngle!='00000000' or HexRaDist!='00000000') and RCS in range(2,128)and PR>90 and cp.fabs(RaVl)<0.003:
					n+=1 
					Az_angle=hex_to_float(HexAzAngle)+AzAngleCor
					El_angle=hex_to_float(HexELAngle)+ElAngCor
					RaDist=hex_to_float(HexRaDist)
					X_pos=RaDist*cp.cos(El_angle)*cp.cos(Az_angle)
					Y_pos=RaDist*cp.cos(El_angle)*cp.sin(Az_angle)
					Z_pos=RaDist*cp.sin(El_angle)
					X_vl=RaVl*cp.cos(El_angle)*cp.cos(Az_angle)
					Y_vl=RaVl*cp.cos(El_angle)*cp.sin(Az_angle)
					Z_vl=RaVl*cp.sin(El_angle)

					Xyz=cp.asarray([X_pos,Y_pos,Z_pos,X_vl,Y_vl,Z_vl]).get()

					point=cp.vstack([point,Xyz]).get()
