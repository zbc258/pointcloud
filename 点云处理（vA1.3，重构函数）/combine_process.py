...
def pack_combine(r_path,w_path):
	...
	point=cp.empty(shape=(0,6),dtype=np.float64).get()
	plt.ion()
	...
	for (ts, buf) in pcap:
		...
		try:
				...
				if result!=b'':#结果非空，说明合并完成
					...
					print(f"第{id}帧：")
					#f2.write(f'在第{id}帧内 \n')
					point=tools.Xyz_compute(result,point,ax)
					...
					if id%size==0:
						...
						tools.set_label(ax)
						# pcd=tools.point_to_pcd([point[:,0],point[:,1],point[:,2]])
						# pcd=filters.pass_through(pcd,limit_min=0,limit_max=25,filter_value_name="z")
						# pcd=filters.pass_through(pcd,limit_min=20,limit_max=500,filter_value_name="x")
